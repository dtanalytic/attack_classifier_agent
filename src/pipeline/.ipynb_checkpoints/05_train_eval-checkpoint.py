...
def main():
    ...
    try:
        ...
        if conf['train_eval']['use_bert']:
            ...
            pred_df = pd.read_csv('data/val_pred.csv').rename(columns={'Unnamed: 0':'index'})
        ...
        if conf['train_eval']['rag_type']=='embed':
            ...
            k = eval(conf['train_eval']['rag_k'])[0]
            retriever = joblib.load(conf['storage']['bm25_path'])
            retriever.k = k
        else:
            ...
            vector_retriever = db.as_retriever(search_type="mmr", search_kwargs={"k": ks[0], 'lambda_mult': conf['train_eval']['lambda_mult'],
                                                                     'fetch_k': conf['train_eval']['fetch_k']})
            bm_retriever = joblib.load(conf['storage']['bm25_path'])
            bm_retriever.k = ks[1]
        ...
        # ----------------------------
        pred_df['enc_ttp'] = pred_df['enc_ttp'].map(lambda x: eval(x))
        ...
        p_macro, r_macro, f1_macro, _ = precision_recall_fscore_support(pred_df['enc_ttp'].tolist(), pred_df['pred_enc_ttp'].tolist(), average='macro',  zero_division=np.nan)
        pred_df.to_csv(conf['train_eval']['pred_df_fn'], index=False)

        d = {'p_micro':p_micro, 'r_micro':r_micro, 'f1_micro':f1_micro, 'p_macro':p_macro, 'r_macro':r_macro, 'f1_macro':f1_macro}
        with open(conf['train_eval']['m_fn'], 'wt') as f:
            json.dump(d, f)

        print(d)
        end = time.time()
    
        logger.info(f"Оценка модели заняла {(end-start)/60:.1f} минут")
